...
def final_detection(main_boxes,total_boxes,each_box_acc,each_box_class_index,height_ratio,width_ratio):
    for k in main_boxes:
        ...
        cv2.putText(image,text,(x,y-3),font,1,(0,0,255),2)
        cv2.rectangle(image,(x,y),(w+x,h+y),(0,255,0),2)

image = cv2.imread('car_image_detection.jpg')
original_image_height,original_image_width = image.shape[0],image.shape[1]
print(f'original Image Height : {original_image_height}')
print(f'original Image width : {original_image_width}')
...
v3_neural_network = cv2.dnn.readNetFromDarknet('yolov3.cfg','yolov3.weights') # loading architecture and weights
...
input_image = cv2.dnn.blobFromImage(image,1/255,(320,320),True,crop=False) # (1,channel,height,width)
