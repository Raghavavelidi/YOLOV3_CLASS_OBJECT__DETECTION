#if person or car will present in frame and detect them
import numpy as np
import cv2
...
def final_detection(main_boxes, total_boxes, each_box_acc, each_box_class_index, height_ratio, width_ratio):
    ...
    for k in main_boxes:
        ...
        # Only process 'car' and 'person', ignore other classes
        if class_name == "car" or class_name == "person":
            # Count how many times 'car' or 'person' is detected
...
# Display the count of 'car' and 'person' only in the top-left corner
    ...
    y_set = 50  # initial offset for the y-axis to print text below each other

# Only display counts for 'car' and 'person'
    for class_name, count in detected_classes.items():
        ...
        y_set += 30  # Update the offset for the next line
